...
std::string masked(std::string_view password) {
...
void LoginView::render(render::GlRenderer& renderer, int width, int height) {
  ...
  renderer.drawText(formX, y + 76.0f, "Choose a user, pick a desktop session, and unlock.", formW, 15, kOnSurfaceVariant);
  ...
  const std::string sessionName = m_sessionIndex < m_sessions.size() ? m_sessions[m_sessionIndex].name : "No sessions";
  ...
  renderer.drawText(m_submitRect.x + 24.0f, m_submitRect.y + 15.0f, "Sign in", m_submitRect.w - 78.0f, 17, kOnPrimary);
  renderer.drawGlyph(m_submitRect.x + m_submitRect.w - 42.0f, m_submitRect.y + 15.0f, "login", 22, kOnPrimary);

  const Color statusColor = m_authenticated ? kTertiary : (m_status.find("failed") != std::string::npos ? kError : kOnSurfaceVariant);
...
void LoginView::drawSelect(render::GlRenderer& renderer, const Rect& rect, std::string_view label, std::string_view value,
                           bool open, bool active) {
  const bool hover = rect.contains(m_pointerX, m_pointerY);
  renderer.drawRect(rect.x, rect.y, rect.w, rect.h, 8.0f, hover ? withAlpha(kOutline, 0.58f) : withAlpha(kSurface, 0.92f),
                    active ? kPrimary : kOutline, active ? 1.5f : 1.0f);
...
void LoginView::drawDropdown(render::GlRenderer& renderer, const Rect& owner, std::string_view label, bool open) {
  ...
  auto& optionRects = label == "User" ? m_userOptionRects : m_sessionOptionRects;
  optionRects.clear();
  renderer.drawRect(owner.x, menuY, owner.w, optionH * static_cast<float>(visible), 10.0f, kSurfaceVariant, kOutline, 1.0f);
  for (std::size_t i = 0; i < visible; ++i) {
    ...
    if (optionHover) {
      renderer.drawRect(option.x + 4.0f, option.y + 4.0f, option.w - 8.0f, option.h - 8.0f, 7.0f, withAlpha(kOutline, 0.65f));
    }
...
void LoginView::drawInput(render::GlRenderer& renderer, const Rect& rect) {
  ...
  renderer.drawText(rect.x + 48.0f, rect.y + 25.0f, m_password.empty() ? "Type password" : masked(m_password),
                    rect.w - 64.0f, 16, m_password.empty() ? kOnSurfaceVariant : kOnSurface);
}
...
void LoginView::keyBackspace() {
  if (m_focus == Field::Password && !m_password.empty()) {
    m_password.pop_back();
  }
}
...
void LoginView::submit() {
  ...
  m_submit = LoginRequest{.user = m_users[m_userIndex].name, .password = m_password, .sessionId = m_sessions[m_sessionIndex].id};
}
